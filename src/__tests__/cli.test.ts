...
describe("parseCommandLineArgs", () => {
  ...
  it("should throw error for missing file path", () => {
    expect(() => parseCommandLineArgs([])).toThrow(
      "Please provide a book file path.",
    )
  })

  it("should throw error for non-string file path", () => {
    expect(() => parseCommandLineArgs([42 as unknown as string])).toThrow(
      "Please provide a book file path.",
    )
  })

  it("should throw UnsupportedFileTypeError for unsupported file type", () => {
    expect(() => parseCommandLineArgs(["/path/to/book.txt"])).toThrow(
      UnsupportedFileTypeError,
    )
  })

  it("should throw error for invalid start page (non-numeric)", () => {
    expect(() => parseCommandLineArgs([validPdfPath, "abc"])).toThrow(
      "Starting page must be a positive integer.",
    )
  })

  it("should throw error for invalid start page (zero)", () => {
    expect(() => parseCommandLineArgs([validPdfPath, "0"])).toThrow(
      "Starting page must be a positive integer.",
    )
  })

  it("should throw error for invalid start page (negative)", () => {
    expect(() => parseCommandLineArgs([validPdfPath, "-1"])).toThrow(
      "Starting page must be a positive integer.",
    )
  })
